...
|   ↳ état courant | Parser `.raw` + `.xdy` produisent désormais un objet `Index` avec `IndexLetterGroup` et `IndexNode` (hiérarchies, locrefs, plages numériques basiques). | Modules `xindy.index.builder`, `.hierarchy`, `.grouping`, `.order`. | Tests unitaires `tests/index/*` basés sur `attr1` et fixtures synthétiques. |
