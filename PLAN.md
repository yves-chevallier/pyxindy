...
|   ↳ état courant | Parser `.raw` + `.xdy` produisent désormais un objet `Index` avec `IndexLetterGroup` et `IndexNode` (hiérarchies, fusion de locrefs, plages numériques basiques). | Modules `xindy.index.builder`, `.hierarchy`, `.grouping`, `.order`. | Tests unitaires `tests/index/*` basés sur `attr1` et fixtures synthétiques. |
|   ↳ prochaines étapes | Étendre les nœuds pour gérer fusion des attributs (`merge-to`), mapping/crossrefs et préparer les compteurs/progressions nécessaires à `markup`. | À implémenter : calcul des pourcentages, support des ranges non numériques, amortissement `sort-rule`. | Nouveaux tests sur `ex1`, `ranges1`, `mappings`. |
|   ↳ reste à faire (étapes détaillées) | 5a. Support de `sort-rule` et des alphabets personnalisés • 5b. Fusion/mapping des attributs (`merge-to`, `map-attributes`) • 5c. Crossrefs (`add-crossref`) et follow-attributes • 5d. Calcul des statistiques (pourcentages, ordres de traitement) • 5e. Couche d’intégration `raw+style` → `Index` pour tous les jeux de tests historiques. | Modules `xindy.index.sorting`, `xindy.index.merge`, `xindy.index.crossref`, instrumentation du builder. | Tests unitaires + premières intégrations (`attr1`, `ex1`, `ranges1`, `mappings`). |
...
|   ↳ sous-étapes | 6a. Implémenter le backend de rendu (structure → flux) • 6b. Support des options `markup-*` et du mode verbose • 6c. Gestion des ranges/crossrefs au rendu • 6d. Snapshots `.ind` pour `ex1`, `ex2`, `deep`. | Module `xindy.markup`. | Tests de rendu + snapshots. |
