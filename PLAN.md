...
|   ↳ état courant | Résolution automatique des modules `.xdy` d’origine (`xindy-src/xindy-2.1/modules`) via `require`; test de charge de module `class/pagenums.xdy` ✅. | Tests `tests/dsl/test_require_modules.py`. | À compléter : snapshots `attr2/attr3`, `ranges1`, langues. |
