...
def test_wegweiser_snapshot():
    state = StyleInterpreter().load(TESTS_DIR / "wegweiser.xdy")
    raw_entries = load_raw_index(TESTS_DIR / "wegweiser.raw")
    index = build_index_entries(raw_entries, state)
    output = render_index(index, style_state=state)
    expected = (TESTS_DIR / "wegweiser.cmp").read_text(encoding="latin-1")
