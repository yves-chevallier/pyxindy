...
def test_deutsch_snapshot():
    state = StyleInterpreter().load(TESTS_DIR / "deutsch.xdy")
    raw_entries = load_raw_index(TESTS_DIR / "deutsch.raw")
    index = build_index_entries(raw_entries, state)
    output = render_index(index, style_state=state)
    expected = (TESTS_DIR / "deutsch.cmp").read_text(encoding="latin-1")
