...
def test_attr2_snapshot():
    state = StyleInterpreter().load(TESTS_DIR / "attr2.xdy")
    raw_entries = load_raw_index(TESTS_DIR / "attr2.raw")
    index = build_index_entries(raw_entries, state)
    output = render_index(index, style_state=state)
    expected = (TESTS_DIR / "attr2.cmp").read_text(encoding="latin-1")
